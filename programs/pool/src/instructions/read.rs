...
use crate::state::{AssetAccount, LpDepositAccount};
use crate::constants::{LP_DEPOSIT_SEED, ASSET_SEED, FEE_SCALE};
...
//
// Formula (matches public_exit logic exactly):
//   fps_delta      = asset.fees_per_share − lp_deposit.fee_debt
...
pub struct GetLpClaimableFees<'info> {
    ...
    /// Asset account for this pool+mint pair — needed for fees_per_share.
    #[account(
        seeds = [
            ASSET_SEED,
            lp_deposit.pool.as_ref(),
...
pub fn handler_get_lp_claimable_fees(
    ...
    let fps_delta    = asset.fees_per_share.saturating_sub(lp.fee_debt);
    ...
    emit!(LpClaimableFees {
        ...
        fees_per_share: asset.fees_per_share,
...
pub struct LpClaimableFees {
    ...
    pub principal:      u64,
    /// Total fee tokens claimable on next public_exit
    pub claimable_fees: u64,
    /// Current global accumulator for this asset
    pub fees_per_share: u64,
    /// LP's debt snapshot (fees accrued before their deposit)
    pub fee_debt:       u64,
}
