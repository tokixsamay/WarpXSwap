...
pub fn handler_check(
    ...
    let (new_fee, old_fee, should_block, was_blocked) = {
        ...
        // Layer 2: Volume — consistently rising (≥10% increase) over 3 periods?
        // Bug #9 fix: passes volume_history [u64; 3] for multi-period check.
        let volume_ok = check_volume_layer(
            asset.pyth_data.volume_24h,
            &asset.pyth_data.volume_history,
        );
