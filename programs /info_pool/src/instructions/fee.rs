...
pub struct CalculateAndPushFee<'info> {
    ...
    /// Only the registered crank keypair may push fees.
    /// UpdatePythFeeds and PushOraclePriceToPool both enforce this constraint;
    /// CalculateAndPushFee must do the same to prevent any signer from
    /// pushing arbitrary fee values to the Pool program.
    #[account(
