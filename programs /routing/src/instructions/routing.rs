...
pub fn handler_find_best(
    ...
    // Bug #6 fix: capture current slot once for oracle staleness checks in the loop.
    ...
    for (i, pool_key) in params.candidate_pools.iter().enumerate() {
        ...
        // Bug #6 fix: skip candidates whose InfoPool oracle data is stale.
        // last_updated == 0 means the price has never been pushed (cold start).
        // Otherwise, skip if the data is older than ORACLE_STALENESS_SLOTS slots
        // (~60 s at 400ms/slot) — routing through stale prices risks fund loss.
        let in_age  = current_slot.saturating_sub(ip_asset_in.pyth_data.last_updated);
        let out_age = current_slot.saturating_sub(ip_asset_out.pyth_data.last_updated);
        if ip_asset_in.pyth_data.last_updated == 0
            || in_age  > ORACLE_STALENESS_SLOTS
            || ip_asset_out.pyth_data.last_updated == 0
            || out_age > ORACLE_STALENESS_SLOTS
        {
            continue;
        }
        ...
        // In find_best, pool_liquidity = ip.pool_size which is total pool value in
        // base-asset terms — not comparable to amount_in (different token units).
        // Pass None so filter_hard_rules skips the vault comparison and relies on
        // the MIN_LIQUIDITY guard only.  The execute path does the real vault check.
        // pool_liquidity is still used below in candidates.push (liquidity field).
...
///
/// `vault_out_balance`: actual balance of the asset_out vault in native token units.
///   Pass the real vault balance when available (execute path) so the liquidity
///   check uses comparable units.  In the find_best path the vault is not passed
///   as an account, so pass `None` — that path skips the direct vault comparison
///   and relies only on MIN_LIQUIDITY and the per-asset execute-path check.
///
/// NOTE: do NOT compare `ip.pool_size` (total pool value in base-asset terms)
/// directly against `amount_in` (native units of a possibly different token) —
/// the denominations are incompatible and produce wrong results for non-base assets.
pub fn filter_hard_rules(
    ...
    // Vault-level liquidity check — only when the actual vault balance is available.
    // When None (find_best path), MIN_LIQUIDITY below is the only size guard.
    if let Some(balance) = vault_out_balance {
        if balance < MIN_LIQUIDITY                       { return false; }
        if balance < amount_out                          { return false; }
    }
